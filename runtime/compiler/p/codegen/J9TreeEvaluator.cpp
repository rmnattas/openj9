...
TR::Register *J9::Power::TreeEvaluator::genCheckCastOrInstanceOfForDynamicClass(TR::Node* node, TR::CodeGenerator* cg, bool isCheckCast, TR::Node* ifInstanceOfNode)
   ...
   // if (!objInDep)
   //    objectReg = nonFixedDependency(deps, objectReg, &depIndex, TR_GPR, true, cg);
   // if (!castClassInDep)
   //    castClassReg = nonFixedDependency(deps, castClassReg, &depIndex, TR_GPR, true, cg);
   // objClassReg = nonFixedDependency(deps, objClassReg, &depIndex, TR_GPR, true, cg);
   // temp1Reg = nonFixedDependency(deps, temp1Reg, &depIndex, TR_GPR, true, cg);
   // temp2Reg = nonFixedDependency(deps, temp2Reg, &depIndex, TR_GPR, false, cg);
   // if (!isCheckCast)
   //    resultReg = nonFixedDependency(deps, resultReg, &depIndex, TR_GPR, false, cg);
   ...
   if (TR::Compiler->om.compressObjectReferences())
      ...
      generateTrg1MemInstruction(cg, TR::InstOpCode::lwz, node, objClassReg,
            TR::MemoryReference::createWithDisplacement(cg, objectReg, (int32_t) TR::Compiler->om.offsetOfObjectVftField(), 4));
   else
      generateTrg1MemInstruction(cg,TR::InstOpCode::Op_load, node, objClassReg,
            TR::MemoryReference::createWithDisplacement(cg, objectReg, (int32_t) TR::Compiler->om.offsetOfObjectVftField(), TR::Compiler->om.sizeofReferenceAddress()));
   ...
   // TODO reduce OOL Dependencies
   // TR_PPCOutOfLineCodeSection *outlinedHelperCall = new (cg->trHeapMemory()) TR_PPCOutOfLineCodeSection(node, (isCheckCast ? TR::call : TR::icall), 
   //    (!isCheckCast ? resultReg : NULL), helperCallLabel, (ifInstanceOfNode ? helperReturnLabel : doneLabel), cg);
